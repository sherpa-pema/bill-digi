-- ==============================================================================
-- DigiBill POS: Self-Healing Bill Sequence & Collision Protection
-- Ensures create_bill_atomic never violates unique key constraints even if counter is mutated
-- ==============================================================================
...
BEGIN
    ...
    -- 7. Self-Healing Sequence Protection
    -- Determine the maximum existing bill number in the bills table for this shop
    SELECT COALESCE(MAX(bill_number) + 1, 1) INTO v_max_existing
    FROM public.bills
    WHERE shop_id = p_shop_id;

    -- Determine safe next bill number: it must be at least the shops.next_bill_number AND strictly greater than all existing bills
    v_bill_number := GREATEST(COALESCE(v_updated_shop.next_bill_number, 1), v_max_existing);

    -- Increment counter atomically in shops table
    UPDATE public.shops
    ...
    -- 8. Insert the bill with the strictly monotonic sequential number
